...
**Última actualización:** 2026-07-13 · **Fase actual:** Fase 2 ✅ completa (código) → arrancando Fase 3 · **Próximo paso:** re-verificar `/admin/event-types` y `/admin/catalog` en vivo tras reiniciar el dev server (caché de Next quedó corrompida por un create→delete rápido de archivo) → luego Fase 3 (Configuración).
...
| 1 | Staff | ✅ | ✅ | ✅ | ✅ |
| 2 | Catálogo | ✅ | ✅ | ✅ | ✅ |
| 2 | Tipos de evento | ✅ | ✅ | ✅ | 🔨 (código OK, re-verificar en vivo post-restart) |
| 3 | Configuración | ☐ | ☐ | ☐ | ☐ |
| 4 | Cotizaciones | ☐ | ☐ | ☐ | ☐ |
| 5 | Eventos | ☐ | ☐ | ☐ | ☐ |
| 6 | Dashboard | ☐ | ☐ | ☐ | ☐ |

**Fundaciones** (transversales): guards nuevos 🔨 (`STAFF`✅ `PRODUCT`✅ `EVENT_TYPE`✅; falta `CONFIG`) · enum `state` compartido ✅ · cents (BE) ✅ + `formatMoney`/`useMoneyFormatter` (FE) ✅ · `%` helper ☐ · dnd-kit ✅ (instalado + `SortableList`/`useSortableRow`/`ReorderControl` en `features/catalog`) · `computeQuoteTotals` ☐
