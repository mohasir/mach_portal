...
**Última actualización:** 2026-07-13 · **Fase actual:** Fase 2 (Catálogo + Tipos de evento) · **Próximo paso:** Fase 2 → Tipos de evento (BE+FE).
...
**Fundaciones** (transversales): guards nuevos 🔨 (`STAFF`✅ `PRODUCT`✅; falta `EVENT_TYPE`/`CONFIG`) · enum `state` compartido ✅ · cents (BE) ✅ + `formatMoney`/`useMoneyFormatter` (FE) ✅ · `%` helper ☐ · dnd-kit ✅ (instalado + `SortableList`/`useSortableRow`/`ReorderControl` en `features/catalog`) · `computeQuoteTotals` ☐
