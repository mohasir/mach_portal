...
export function ClientForm({ initialValues, onSubmit, isPending }: ClientFormProps) {
  ...
  return (
    ...
    >
      ...
      <Form.Item name="address" label={t('form.address')} rules={[{ max: 240 }]}>
        <Input.TextArea placeholder={t('form.addressPlaceholder')} autoSize={{ minRows: 1, maxRows: 3 }} />
      </Form.Item>

      <Form.Item name="notes" label={t('form.notes')} rules={[{ max: 2000 }]}>
        <Input.TextArea placeholder={t('form.notesPlaceholder')} autoSize={{ minRows: 2, maxRows: 5 }} />
      </Form.Item>
